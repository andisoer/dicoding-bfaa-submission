            ...
            override fun onResponse(call: Call<UserDetail>, response: Response<UserDetail>) {
                when {
                    response.isSuccessful -> {
                        when {
                            response.body() != null -> {
                                detailLiveData.postValue(Status.success(data = response.body()))
                            } else -> {
                                detailLiveData.postValue(Status.error(message = "Empty data", data = null))
                            }
                        }
                    } else -> {
                        detailLiveData.postValue(Status.error(message = "Failed to search user", data = null))
                    }
                }
            }

            override fun onFailure(call: Call<UserDetail>, t: Throwable) {
                detailLiveData.postValue(Status.error(message = "Error, ${t.message}", data = null))
            }
        })
